...
df.columns = ['ds', 'y','PRECIPITACION', 'FERIADO', 'LABORABLE']
df.head()
...
for param in params:
    m = Prophet(**param)
    m.add_regressor('PRECIPITACION')
    m.add_regressor('FERIADO')
    m.add_regressor('LABORABLE')
...
future['PRECIPITACION'] = df['PRECIPITACION'].values[:len(future)]
future['FERIADO'] = df['FERIADO'].values[:len(future)]
future['LABORABLE'] = df['LABORABLE'].values[:len(future)]
