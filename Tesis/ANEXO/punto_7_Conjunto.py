...
df = pd.read_csv('fechas_corregidas.csv')


# Cargamos los datos desde un archivo CSV
datos = pd.read_csv('fechas_corregidas.csv')
...
plt.title('Cambio en el Precio a lo largo de los 4 años')
plt.xlabel('Fecha')
plt.ylabel('Precio')
plt.legend()
plt.grid(True)
...
plt.title('Demanda del Total de Pasajes, linea A a lo largo de los 4 años')
plt.xlabel('Fecha')
plt.ylabel('TOTAL')
plt.legend()
plt.grid(True)
...
plt.title('Regresión Lineal entre TOTAL (demanda pasajes) y PRECIO')
plt.xlabel('TOTAL')
plt.ylabel('PRECIO')
plt.legend()
plt.show()
